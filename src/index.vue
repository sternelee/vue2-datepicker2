  ...
  <div
    ...
    <div class="mx-datepicker-popup"
      ...
      <div class="mx-range-wrapper"
        :style="{width: 248*rangeColumn + 'px'}"
        v-else>
        <calendar-panel
          style="box-shadow:1px 0 rgba(0, 0, 0, .1)"
          v-bind="$attrs"
          v-for="(n, k) in rangeColumn"
          :key="k"
          :ref="`calendarPanel${k}`"
          :index="k"
          :type="innerType"
...
export default {
  ...
  data () {
    ...
    const rangeMonths = []
    const rangeYears = []
    const rangeVals = Array.from({length: this.rangeColumn}, (v, k) => null)
  ...
  methods: {
    initCalendar () {
      ...
      this.displayPopup()
      this.rangeVals = Array.from({length: this.rangeColumn}, (v, k) => null)
    },
    stringify (date) {
